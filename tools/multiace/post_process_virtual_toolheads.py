...
# FIX (2026-08-21): the script prints Unicode symbols (e.g. the '→'
# arrow in the "suggested rearrangement" report). When run as a plain
# subprocess on Windows (exactly how Orca's Post-processing Scripts feature
# invokes it), stdout/stderr default to the legacy console codepage
# (cp1252), which can't encode that character - UnicodeEncodeError crashed
# the whole run. Force UTF-8 with a safe fallback so no output can ever
# crash the process regardless of how the parent invokes it.
...
def rewrite(gcode, feeder_heads=frozenset()):
    """feeder_heads: head indices that are pinned feeders, not ACE-fed.
    They must never get an ACE_SWAP_HEAD - the bare T<head> command below
    unconditionally assumed every low index (0-3) was ACE0/Slot<head> and
    stamped a swap-back onto it regardless (FIX 2026-08-21: this produced
    e.g. "ACE_SWAP_HEAD HEAD=1 ACE=0 SLOT=1" for a head that is actually a
    plain PLA feeder with no ACE wiring at all)."""
    ...
    while i < len(lines):
        ...
        if m_t:
            ...
            if head in feeder_heads:
                # Pinned feeder head - nothing to swap, just select it.
...
def apply_head_mode_rewrite(gcode, assignment):
    """FIX (2026-08-21b): Head-mode-safe replacement for the
    apply_remap()+rewrite() pair. Those route everything through a single
    integer T-index (ace*4+slot for ACE colours, bare head number for
    pinned feeders) - with a real ACE at index 0 (the common case, one ACE
    unit), its slot-encoded values (0-3) land in the EXACT SAME numeric
    range as physical feeder head numbers (also 0-3), so an ACE colour and
    a feeder colour can silently collide onto the same rewritten T-index.
    Confirmed happening on a real file: T1 (meant ACE 0 / Slot 1) collided
    with a feeder pinned to head 1, so the ACE swap for T1 was silently
    dropped and the printer would have extruded the feeder's material
    instead. This function never round-trips through that integer -  it
    walks the gcode using the ORIGINAL slicer T-index (read off the
    "; Change Tool X -> Tool Y" comments, the same source of truth
    parse_toolchanges() uses) and looks up its real (kind, head, ace, slot)
    directly in `assignment` for every substitution, so no two different
    original tools can ever be confused with each other regardless of what
    numbers happen to coincide.

    assignment: {orig_t: {'kind': 'pin'|'ace'|'none', 'head': int,
                          'ace': int, 'slot': int}}, as returned by
    compute_head_mode_layout()['assignment'].

    Only rewrites within the print BODY (from the first
    "; Change Tool" marker onward) - mirrors rewrite()'s existing pre/body
    split, deliberately leaving the start-gcode's early bare T<initial>
    command(s) untouched (those can run before G28/homing, where a
    mechanical ACE swap is unsafe; the safe initial load is injected
    separately by inject_auto_load()).

    Returns (gcode, active_swaps).
    """
    change_re = re.compile(r'^;\s*Change Tool\s*\d+\s*->\s*Tool\s*(\d+)')
    bare_t_re = re.compile(r'^T(\d{1,2})\s*$')
    preextr_re = re.compile(r'^SM_PRINT_PREEXTRUDE_FILAMENT INDEX=(\d+)\b')
    ...
    for line in lines:
        s = line.strip()
        m_chg = change_re.match(s)
        if m_chg:
            pending_t = int(m_chg.group(1))
            out.append(line)
            continue

        m_t = bare_t_re.match(s)
        if m_t:
            orig_t = pending_t if pending_t is not None else int(m_t.group(1))
            e = assignment.get(orig_t)
            head = e['head'] if e and e.get('kind') in ('pin', 'ace') else int(m_t.group(1))
            out.append('T%d' % head)
            if e and e.get('kind') == 'ace':
                key = (e['ace'], e['slot'])
                if head_loaded.get(head) != key:
                    out.append('ACE_SWAP_HEAD HEAD=%d ACE=%d SLOT=%d' % (
                        head, e['ace'], e['slot']))
                    head_loaded[head] = key
                    swaps += 1
            pending_t = None
        ...
        m_pre = preextr_re.match(s)
        if m_pre:
            t = int(m_pre.group(1))
            e = assignment.get(t)
            if e and e.get('kind') == 'ace':
                # Dropped: a real ACE swap flushes/primes on its own.
                continue
            if e and e.get('kind') == 'pin':
                out.append('SM_PRINT_PREEXTRUDE_FILAMENT INDEX=%d' % e['head'])
                continue
            out.append(line)
            continue
        ...
        if m104_re.match(s):
            def _sub_t(tm):
                t = int(tm.group(1))
                h = _head_for(t)
                return 'T%d' % h if h is not None else tm.group(0)
            out.append(re.sub(r'T(\d{1,2})', _sub_t, line))
            continue
...
def _orca_config_dir():
    """Best-effort locate the OrcaSlicer config directory (where
    OrcaSlicer.conf and the system/user preset trees live)."""
    if os.name == 'nt':
        appdata = os.environ.get('APPDATA')
        if appdata:
            return os.path.join(appdata, 'OrcaSlicer')
...
def find_orca_print_host(max_depth=6):
    """Read the IP/hostname the user already configured in Orca's own
    Physical Printer / printer-connection settings for the CURRENTLY
    active machine preset, so --live-lookup doesn't need the address
    typed a second time. Walks the preset's 'inherits' chain (a bare
    Snapmaker system profile usually keeps print_host on the leaf user
    preset, but this covers it either way). Returns the host string, or
    None if it can't be resolved (caller should fall back)."""
    cfg_dir = _orca_config_dir()
    if not cfg_dir:
        return None
    conf_path = os.path.join(cfg_dir, 'OrcaSlicer.conf')
    try:
        with open(conf_path, 'r', encoding='utf-8-sig', errors='replace') as f:
            text = f.read()
        # OrcaSlicer.conf has a trailing "# MD5 checksum ..." comment line
        # after the closing brace - not valid JSON. raw_decode() parses just
        # the JSON object and ignores whatever trails it.
        conf, _ = json.JSONDecoder().raw_decode(text)
    except (OSError, ValueError):
        return None
    preset_name = (conf.get('presets') or {}).get('machine')
    if not preset_name:
        return None

    def _find_preset_file(name):
        user_path = os.path.join(cfg_dir, 'user', 'default', 'machine',
                                 name + '.json')
        if os.path.isfile(user_path):
            return user_path
        system_dir = os.path.join(cfg_dir, 'system')
        if os.path.isdir(system_dir):
            for vendor in os.listdir(system_dir):
                candidate = os.path.join(system_dir, vendor, 'machine',
                                         name + '.json')
    ...
    for _ in range(max_depth):
        ...
        path = _find_preset_file(preset_name)
        if not path:
            break
        try:
            ...
            break
        host = (preset.get('print_host') or '').strip()
        if host:
            return host
        preset_name = preset.get('inherits')
    return None
...
def lookup_head_mode_context(host, port=80, path='/multiace/api/state', timeout=5.0):
    """Query the same /multiace/api/state endpoint as lookup_live_slots, but
    extract the head-mode context: whether the printer is in 'head' mode
    (feeders pinned to fixed heads + ACE slots multiplexed on the rest), the
    feeders' loaded material/colour, and the ACE-head wiring. Mirrors
    main.py's _head_mode_context(). Returns None on any HTTP/parse error, or
    a dict:
        {'mode': str, 'feeders': [{'head','material','color'}, ...],
         'ace_heads': [int,...], 'head_ace': {head: ace_index}}
    (a non-'head' mode still returns a dict; callers should check mode)."""
    if ':' in host:
        host, _, port_str = host.partition(':')
        try:
            port = int(port_str)
    ...
    try:
        ...
        print('WARNING: head-mode lookup to %s failed: %s' % (url, e),
              file=sys.stderr)
        return None
    mode = data.get('mode') or 'normal'
    feeders = []
    for th in data.get('toolheads', []) or []:
    ...
    ace_heads = [int(h) for h in (data.get('ace_heads') or [])]
    if not ace_heads:
        ace_heads = [int(data.get('ace_head', 3) or 3)]
    raw_head_ace = data.get('head_ace', {}) or {}
    head_ace = {}
    for h in range(4):
        try:
            ...
            head_ace[h] = h
    return {'mode': mode, 'feeders': feeders, 'ace_heads': sorted(set(ace_heads)),
            'head_ace': head_ace}
...
def inject_auto_load(gcode, feeder_heads=frozenset(), initial_targets=None):
    ...
    for i in range(body_start, len(lines)):
        ...
        if m_t:
            ...
            if head not in initial:
                ...
                elif initial_targets and head in initial_targets:
                    initial[head] = initial_targets[head]
                else:
                    initial[head] = (0, head)
            continue
        m = re.match(
    ...
    for head in used_heads:
        if head not in initial:
            # FIX (2026-08-21b): the (0, head) fallback assumes ACE index ==
            # physical head number, which only holds by coincidence. Use the
            # real (ace, slot) this head actually needs first, when known.
            if initial_targets and head in initial_targets:
                initial[head] = initial_targets[head]
            else:
                initial[head] = (0, head)
    # FIX (2026-08-21): a pinned feeder head has no ACE wiring at all - it
    # must never get an injected ACE_SWAP_HEAD (this is what produced e.g.
    # "ACE_SWAP_HEAD HEAD=2 ACE=0 SLOT=2" for a plain PLA feeder head).
    for head in feeder_heads:
        initial.pop(head, None)
...
def main():
    ...
    if '--live-lookup' in args:
        ...
        # FIX (2026-08-21): the file-extension check below used to decide
        # whether the next token was an IP or the gcode path. That breaks
        # when Orca runs post-processing on its own internal temp file
        # during a direct Upload-and-Print (named e.g.
        # ".OrcaSlicer.upload.<hash>", no .gcode extension) - the temp
        # path itself got misread as an IP, both tokens were consumed, and
        # main() crashed with no filepath left (IndexError -> exit code 1
        # in Orca, no useful message shown). Orca always appends the
        # target file as the LAST argument, so use position instead of the
        # filename: next_arg is a real value only if something else still
        # follows it (the actual file).
        next_arg = args[i + 1] if i + 1 < len(args) else None
        has_more_after = (i + 2) < len(args)
        if next_arg and has_more_after:
            live_lookup_host = next_arg
            del args[i:i + 2]
        else:
            del args[i]
            # FIX (2026-08-21): no IP given on the command line - read the
            # one already configured in Orca's own Physical Printer
            # settings for the active preset, so the address only has to
            # be typed once (in Orca), not again in the Post-processing
            # Scripts field. Falls back to MULTIACE_HOST / 127.0.0.1 if
            # Orca's config can't be found or parsed.
            orca_host = find_orca_print_host()
            if orca_host:
                live_lookup_host = orca_host
                print("Using printer address from Orca's printer settings: %s"
                      % orca_host)
            else:
                live_lookup_host = os.environ.get('MULTIACE_HOST', '127.0.0.1')
                print("Could not read Orca's printer settings - using %s "
                      "(set MULTIACE_HOST or pass --live-lookup <ip> instead)"
                      % live_lookup_host)
    ...
    if '--fuzzy-color' in args:
        ...
        next_arg = args[i + 1] if i + 1 < len(args) else None
        # FIX (2026-08-21): same file-extension-vs-position bug as
        # --live-lookup above - use position, not filename, to tell a
        # numeric value apart from the (possibly extension-less) target
        # file Orca appends.
        has_more_after = (i + 2) < len(args)
        if next_arg and has_more_after:
            try:
    ...
    if live_lookup_host is not None:
        ...
        head_ctx = lookup_head_mode_context(live_lookup_host)
        feeders = (head_ctx or {}).get('feeders') or []
        head_mode = bool(head_ctx) and head_ctx.get('mode') == 'head'

        if not live_slots and not feeders:
            print('ERROR: live-lookup returned 0 loaded slots/feeders - load '
                  'filaments first (e.g. ACEB__Load_All) and try again.',
                  file=sys.stderr)
            sys.exit(1)

        print('Read printer loadout (live, %d ACE slot(s)%s):' % (
            len(live_slots),
            ', %d feeder(s)' % len(feeders) if feeders else ''))
        ...
        for f in sorted(feeders, key=lambda x: x['head']):
            color_str = format_color_hex_rgb(f['color']) if f['color'] else '(no color)'
            material = f['material'] or '?'
            print('  Feeder (head %d)  %-6s %s' % (f['head'], material, color_str))
        slicer_colors = parse_color_names(gcode)
        slicer_types = parse_filament_types(gcode)

        # DIAGNOSTIC/FIX (2026-08-21): feeders carry filament too - a
        # material only loaded on a feeder (not any ACE slot) must not be
        # reported as "missing" here, or head-mode prints that rely on a
        # pinned feeder for one material always fail this check.
        avail_slots = list(live_slots) + [
            {'material': f['material']} for f in feeders]
        missing_mats = check_material_availability(slicer_types, avail_slots)
        if missing_mats:
        ...
        if head_mode:
            # FIX (2026-08-21): the printer is in Head mode (feeders pinned to
            # fixed heads + one ACE multiplexed on the rest) - the simple
            # match_colors_to_slots() below has no concept of a pinned feeder
            # head, so it can never place a colour that only lives on a
            # feeder (this is exactly what made --live-lookup fail on a
            # working printer: it saw PLA nowhere because it never looked at
            # feeders). Use the same compute_head_mode_layout() the web
            # preflight's "loadout" plan uses instead.
            ace_heads = head_ctx['ace_heads']
            head_ace = head_ctx['head_ace']
            ace_head_of_ace = {}
            for h in ace_heads:
                ace_head_of_ace[head_ace.get(h, h)] = h
            layout = compute_head_mode_layout(
                slicer_colors, slicer_types, feeders, live_slots,
                ace_head_of_ace, fuzzy_max_distance=fuzzy_max_distance)
            assignment = layout['assignment']
            # FIX (2026-08-21): tell rewrite()/inject_auto_load() which heads
            # are pinned feeders so they never get a bogus ACE_SWAP_HEAD.
            feeder_heads_used = set(layout.get('pinned') or [])

            print('Head-mode match (slicer T -> pinned feeder / ACE head+slot):')
            # FIX (2026-08-21b): do NOT encode through a single ace*4+slot /
            # head integer here - with a real ACE at index 0 (the common
            # case), that value range collides with physical feeder head
            # numbers (both land in 0-3), so two different original tools
            # could round-trip to the identical rewritten T-index and get
            # silently confused. live_remap stays empty (apply_remap() is
            # skipped below); apply_head_mode_rewrite() uses `assignment`
            # directly instead, keyed by the ORIGINAL slicer T-index, so no
            # such collision is possible.
            live_remap = {}
            any_warn = False
            for t in sorted(assignment.keys()):
                ...
                if e['kind'] == 'pin':
                    print('  T%-2d %-6s %s  ->  Feeder head %d' % (
                        t, mat, format_color_hex_rgb(hex_c), e['head']))
                elif e['kind'] == 'ace':
                    print('  T%-2d %-6s %s  ->  ACE %d Slot %d (head %d)' % (
                        t, mat, format_color_hex_rgb(hex_c), e['ace'], e['slot'],
                        e['head']))
                else:
                    any_warn = True
                    print('  ! T%-2d %-6s %s  ->  (no pinned feeder or ACE slot available)' % (
                        t, mat, format_color_hex_rgb(hex_c)))
            if not layout['feasible']:
                print('ERROR: too few loaded feeders/slots for the colours this '
                      'gcode needs - load more filament and re-run.',
                      file=sys.stderr)
                sys.exit(1)
            if any_warn:
                print('  Note: entries marked ! could not be placed.')

            gcode, head_mode_swaps = apply_head_mode_rewrite(gcode, assignment)
            if head_mode_swaps:
                print('Head-mode rewrite: %d ACE swap(s) written into the body'
                      % head_mode_swaps)
            # Fallback initial-load target per head, for inject_auto_load()
            # below, in case the body scan doesn't find an explicit swap to
            # read the real (ace, slot) off (see FIX 2026-08-21b there too).
            initial_targets = {
                e['head']: (e['ace'], e['slot'])
                for e in assignment.values() if e.get('kind') == 'ace'
            }
    ...
    if head_mode:
        # FIX (2026-08-21b): apply_head_mode_rewrite() above already fully
        # rewrote the body (bare T, M104/M109, SM_PRINT_PREEXTRUDE_FILAMENT,
        # ACE_SWAP_HEAD insertion) directly from `assignment`. Do NOT also
        # run the generic rewrite() here - its body swap-back logic assumes
        # ACE=0/SLOT=head for any bare T<0-3> with no explicit swap right
        # after it, which is exactly the collision this whole fix avoids;
        # running it again on already-correct output risks re-introducing
        # a wrong swap or a redundant one from a fresh, uninformed
        # head_loaded guess.
        pass
    else:
        gcode, active_swaps, skipped_swaps, swapback_count = rewrite(
            gcode, feeder_heads=feeder_heads_used)
    ...
    auto_load_count = 0
    if auto_load:
        gcode, auto_load_count = inject_auto_load(
            gcode, feeder_heads=feeder_heads_used,
