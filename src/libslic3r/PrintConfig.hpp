...
PRINT_CONFIG_CLASS_DERIVED_DEFINE(
    ...
    ((ConfigOptionBool,               multiace_native_postprocess))
...
static std::vector<T> get_flush_volumes_matrix(const std::vector<T> &fv_matrix, size_t extruder_id = -1, size_t nozzle_nums = 1)
{
    if (extruder_id != -1 && nozzle_nums != 1) {
        size_t chunk_size = fv_matrix.size() / nozzle_nums;
        size_t begin_idx  = size_t(chunk_size * extruder_id + EPSILON);
        size_t end_idx    = size_t(chunk_size * (extruder_id + 1) + EPSILON);
        // FIX (2026-08-21): fv_matrix.size() can be stale relative to the current filament
        // count / nozzle_nums (e.g. right after filaments are added/changed and the flush
        // matrix hasn't been re-sized yet), which made begin_idx/end_idx run past
        // fv_matrix.end() and construct an out-of-range vector -- surfaced as a
        // "vector too long" crash when slicing a MultiACE project with mixed filament
        // materials. Clamp into range instead of crashing; a short/zeroed flush matrix here
        // just means default (0) purge volumes for the affected nozzle, not a crash.
        if (begin_idx > fv_matrix.size())
            begin_idx = fv_matrix.size();
        if (end_idx > fv_matrix.size())
            end_idx = fv_matrix.size();
        if (begin_idx > end_idx)
            begin_idx = end_idx;
        return std::vector<T>(fv_matrix.begin() + begin_idx, fv_matrix.begin() + end_idx);
    }
    return fv_matrix;
}
