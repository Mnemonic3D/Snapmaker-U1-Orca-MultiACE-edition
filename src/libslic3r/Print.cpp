...
StringObjectException Print::validate(StringObjectException *warning, Polygons* collison_polygons, std::vector<std::pair<Polygon, float>>* height_polygons) const
{
    ...
    if (this->has_wipe_tower() && ! m_objects.empty()) {
        ...
        // FIX (2026-08-21): this check assumes "single_extruder_multi_material" means a real
        // single physical nozzle with nothing else to keep cool while idle, so ooze prevention
        // is meaningless there. That assumption doesn't hold for MultiACE: SEMM is used here as
        // a slicing abstraction (filament routing is actually handled by ACE post-processing),
        // while the U1 toolchanger has real physical nozzles sitting idle between activations
        // that do benefit from ooze prevention - it's needed on Snapmaker's own stock U1 profile
        // for exactly this reason. Removed the block so both can be enabled together.
...
void Print::_make_wipe_tower()
{
    ...
    if (!is_wipe_tower_type2) {
        ...
        for (size_t nozzle_id = 0; nozzle_id < nozzle_nums; ++nozzle_id) {
            ...
            // FIX (2026-08-21): flush_matrix should hold number_of_extruders*number_of_extruders
            // entries, but can come back short if the stored flush_volumes_matrix is stale
            // relative to the current filament count (see get_flush_volumes_matrix). Pad with 0
            // (no purge) instead of reading/constructing past the end of the vector, which
            // previously crashed ("vector too long") on MultiACE projects mixing filament
            // materials.
            size_t expected_size = size_t(number_of_extruders) * size_t(number_of_extruders);
            if (flush_matrix.size() < expected_size) {
                BOOST_LOG_TRIVIAL(error) << "Print::_make_wipe_tower: flush_matrix for nozzle " << nozzle_id
                    << " has size " << flush_matrix.size() << ", expected " << expected_size
                    << " (number_of_extruders=" << number_of_extruders << "); padding with zeros";
                flush_matrix.resize(expected_size, 0.f);
            }
