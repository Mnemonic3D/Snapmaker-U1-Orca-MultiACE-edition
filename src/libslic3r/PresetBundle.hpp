...
public:
    ...
    unsigned int sync_ams_list(std::vector<std::pair<DynamicPrintConfig *,std::string>> &unknowns, bool use_map, std::map<int, AMSMapInfo> &maps, bool enable_append, MergeFilamentInfo &merge_info, bool color_only = false, bool exact_count = false);
    ...
    void                        update_multi_material_filament_presets(size_t to_delete_filament_id = size_t(-1), bool expand_to_extruders = true);
