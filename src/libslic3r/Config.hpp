...
//        assert(opt_default == nullptr || dynamic_cast<ConfigOptionVector<T>>(opt_default));
    ...
    bool apply_override(const ConfigOption *rhs, std::vector<int>& default_index) override {
        ...
        // DIAGNOSTIC (2026-08-20): tracing "vector too long" / bad_alloc crash on mixed-material
        // MultiACE prints. rhs_vec->size() should be a small filament/extruder count; if it is
        // absurdly large, something upstream corrupted it (or this is not really the vector we
        // think it is). Log it and refuse to resize rather than crash.
        if (rhs_vec->size() > 4096 || this->size() > 4096) {
            BOOST_LOG_TRIVIAL(error) << "ConfigOptionVector::apply_override, Line " << __LINE__
                << ": DIAGNOSTIC refusing oversized resize -- this->size()=" << this->size()
                << " rhs_vec->size()=" << rhs_vec->size()
                << " default_index.size()=" << default_index.size();
            return false;
        }
        ...
        if (default_index.size() != rhs_vec->size()) {
            BOOST_LOG_TRIVIAL(error) << "ConfigOptionVector::apply_override, Line " << __LINE__
                << ": DIAGNOSTIC default_index.size()=" << default_index.size()
                << " != rhs_vec->size()=" << rhs_vec->size();
        }
        // DIAGNOSTIC (2026-08-20): default_value can legitimately be empty here (this->values
        // started empty). The original code unconditionally read default_value[0] in that case,
        // which is undefined behavior on an empty vector -- tracing a "bad allocation" crash
        // seen only when mixing filament materials across a MultiACE printer's extruders.
        if (default_value.empty()) {
            BOOST_LOG_TRIVIAL(error) << "ConfigOptionVector::apply_override, Line " << __LINE__
                << ": DIAGNOSTIC default_value is EMPTY, rhs_vec->size()=" << rhs_vec->size()
                << ", this->size() after resize=" << this->values.size();
        }
        ...
        for (size_t i = 0; i < rhs_vec->size(); ++i) {
            if (!rhs_vec->is_nil(i)) {
                ...
                BOOST_LOG_TRIVIAL(error) << "ConfigOptionVector::apply_override, Line " << __LINE__
                    << ": DIAGNOSTIC skipping index " << i << " -- default_value empty, would have crashed";
                // leave this->values[i] at its resized default (value-initialized / copy of nothing)
            } else {
                if ((i < default_index.size()) && (default_index[i] - 1 >= 0) && (static_cast<size_t>(default_index[i] - 1) < default_value.size()))
