...
void PrintConfigDef::init_common_params()
{
    ...
    def->sidetext = u8"Â°";	// degrees, don't need translation
    def->max = 360;
    def->min = -360;
    def->mode = comAdvanced;
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->sidetext = L(u8"\u2103" /* Â°C */);	// degrees Celsius, CIS languages need translation
    ...
    def->tooltip = L("External Bridging angle override.\n"
        ...
        "Use 180Â° for zero absolute angle.");
    def->sidetext = u8"Â°";	// degrees, don't need translation
    def->min = 0;
    def->max = 180;
    def->mode = comAdvanced;
    ...
    def->sidetext = L(u8"mm/sÂ²");	// millimeters per second per second, CIS languages need translation
    def->min = 0;
    def->mode = comAdvanced;
    ...
    def->min = 0;
    def->max = 200;
    def->sidetext = L(u8"mmÂ³/s");	// cubic millimeters per second, CIS languages need translation
    def->set_default_value(new ConfigOptionFloatsNullable{ 0 });
    ...
    */
    ...
    def->sidetext = L(u8"mmÂ³");	// cubic millimeters, CIS languages need translation
    def->min = 0;
    def->mode = comAdvanced;
    ...
    def->sidetext = L(u8"mmÂ²");	// square millimeters, CIS languages need translation
    def->min = 0;
    def->mode = comAdvanced;
    ...
    def->sidetext = L(u8"g/cmÂ³");	// grams per cubic centimeter, CIS languages need translation
    def->min = 0;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloats { 0. });
    ...
    def->sidetext = L(u8"mmÂ³/sÂ²");	// cubic millimeters per second per second, CIS languages need translation
    def->min = 0;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionFloat(0));
    ...
    // FIX (2026-08-21): native in-process replacement for the external
    // post_process_virtual_toolheads.py/.exe helper - see
    // src/libslic3r/MultiACE/ and src/slic3r/Utils/MultiACE/. Gated behind
    // this explicit, default-off option rather than being wired in
    // unconditionally: it touches the same upload/export pipeline every
    // printer goes through, and must never change behavior for anyone who
    // hasn't deliberately opted in on a MultiACE printer preset.
    def = this->add("multiace_native_postprocess", coBool);
    def->label = L("Native MultiACE post-processing");
    def->tooltip = L("Rewrite ACE color-swap tool changes in-process instead of "
                   "invoking the external post-processing script. Only meaningful "
                   "on a MultiACE printer with print_host set.");
    def->mode = comDevelop;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->sidetext = L(u8"\u2206\u2103" /* âˆ†Â°C */);	// delta degrees Celsius, CIS languages need translation
    def->min = -max_temp;
    def->max = max_temp;
    def->mode = comAdvanced;
...
//    def->min = 0;
    ...
    def->tooltip = L("This is the chamber temperature at which printing should start, while the chamber continues heating "
                     ...
                     "begin printing once the chamber reaches 50Â°C, without waiting for the full 60Â°C.\n\n"
    ...
    // FIX (2026-08-21): MultiACE physically swaps filament via the ACE hardware rather
    // than relying on wipe-tower purging, so the stock 280mm^3 default is unnecessary
    // for this fork - default to a minimal 1mm^3 off-diagonal value instead.
    def->set_default_value(new ConfigOptionFloats {   0.f,   1.f,   1.f,   1.f,
                                                      1.f,   0.f,   1.f,   1.f,
                                                      1.f,   1.f,   0.f,   1.f,
                                                      1.f,   1.f,   1.f,   0.f });
...
void DynamicPrintConfig::update_values_to_printer_extruders_for_multiple_filaments(DynamicPrintConfig& printer_config, std::set<std::string>& key_set, std::string id_name, std::string variant_name)
{
    ...
    if ((extruder_count > 1) || different_extruder)
    {
        ...
        // DIAGNOSTIC (2026-08-20): tracing "vector too long" crash on mixed-material MultiACE
        // prints -- filament_count feeds several new_values.resize(filament_count) calls below;
        // if it's absurd, bail out instead of crashing.
        if (filament_count > 4096) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: DIAGNOSTIC refusing oversized filament_count=%2%")%__LINE__%filament_count;
            return;
        }
        //apply process settings
        //auto opt_nozzle_diameters = this->option<ConfigOptionFloats>("nozzle_diameter");
...
{
    // MultiACE dynamic inherited extruder count
    // A child printer may intentionally expose fewer logical tools than its
    // inherited maximum. Resize first so set_only_diff() can merge into the
    // requested layout instead of retaining the parent's vector length.
    if (extruder_id_name == "printer_extruder_id") {
        auto *child_nozzles  = new_config.option<ConfigOptionFloats>("nozzle_diameter");
        auto *child_ids      = new_config.option<ConfigOptionInts>("printer_extruder_id");
        auto *child_variants = new_config.option<ConfigOptionStrings>("printer_extruder_variant");
        auto *base_nozzles   = this->option<ConfigOptionFloats>("nozzle_diameter");

        if (child_nozzles && child_ids && child_variants && base_nozzles) {
            const size_t child_count = child_nozzles->values.size();
            if (child_count > 0 &&
                child_ids->values.size() == child_count &&
                child_variants->values.size() == child_count &&
                child_count < base_nozzles->values.size()) {
                this->set_num_extruders(child_count);
            }
        }
    }
    std::vector<int> cur_extruder_ids, target_extruder_ids, variant_index;
...
// DIAGNOSTIC (2026-08-20): small helper to render a vector<int> for logging, avoids pulling in extra boost headers.
static std::string diagnostic_join_ints(const std::vector<int>& v)
{
    std::string out;
...
{
    // DIAGNOSTIC (2026-08-20): tracing "bad allocation" crash reported when mixing filament
    // materials (e.g. PETG + PLA) across a MultiACE printer's physical extruders.
    if (!opt_new_machine || !opt_old_machine) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: DIAGNOSTIC null machine option for key '%2%' (opt_new_machine=%3%, opt_old_machine=%4%), f_maps=[%5%]")
            % __LINE__ % opt_key % (opt_new_machine != nullptr) % (opt_old_machine != nullptr)
            % diagnostic_join_ints(f_maps);
        return;
    }
    ...
    if (opt_key == "long_retractions_when_cut" && new_full_config.option<ConfigOptionInt>("enable_long_retraction_when_cut")->value != LongRectrationLevel::EnableFilament) {
        ...
        if (!ptr) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: DIAGNOSTIC dynamic_cast to ConfigOptionBoolsNullable failed for key '%2%'")%__LINE__%opt_key;
            return;
        }
    ...
    if (opt_key == "retraction_distances_when_cut" && new_full_config.option<ConfigOptionInt>("enable_long_retraction_when_cut")->value != LongRectrationLevel::EnableFilament) {
        ...
        if (!ptr) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: DIAGNOSTIC dynamic_cast to ConfigOptionFloatsNullable failed for key '%2%'")%__LINE__%opt_key;
            return;
        }
        for (size_t idx = 0; idx < ptr->values.size(); ++idx)
            opt_retraction_distance_default.values.push_back(ptr->nil_value());
        opt_new_filament = &opt_retraction_distance_default;
    }

    {
        auto vec_new_machine = dynamic_cast<const ConfigOptionVectorBase*>(opt_new_machine);
        auto vec_new_filament = dynamic_cast<const ConfigOptionVectorBase*>(opt_new_filament);
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(", Line %1%: DIAGNOSTIC key='%2%' opt_new_machine->size=%3% opt_new_filament->size=%4% f_maps=[%5%]")
            % __LINE__ % opt_key % (vec_new_machine ? (int)vec_new_machine->size() : -1) % (vec_new_filament ? (int)vec_new_filament->size() : -1)
            % diagnostic_join_ints(f_maps);
    }

    ConfigOption* opt_copy = nullptr;
    try {
        opt_copy = opt_new_machine->clone();
        opt_copy->apply_override(opt_new_filament, f_maps);
    } catch (const std::exception& ex) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(", Line %1%: DIAGNOSTIC exception in clone/apply_override for key '%2%': %3%")%__LINE__%opt_key%ex.what();
        delete opt_copy;
        return;
    }
