...
unsigned int PresetBundle::sync_ams_list(std::vector<std::pair<DynamicPrintConfig *,std::string>> &unknowns, bool use_map, std::map<int, AMSMapInfo> &maps, bool enable_append, MergeFilamentInfo &merge_info, bool color_only, bool exact_count)
{
    ...
    for (auto &entry : filament_ams_list) {
        ...
        if (filament_id.empty()) {
            if (use_map) {
                ...
                // MultiACE: retain the available ACE slot color while unloaded.
                ...
                if (filament_multi_color.empty() && !filament_color.empty())
    ...
    if (color_only) {
        ...
        if (has_placeholders) {
            ...
            size_t total      = exact_count ? tray_count : std::max(tray_count, exist_presets.size());
            ...
            for (size_t i = 0; i < total; i++) {
                ...
                if (is_loaded) {
                    ...
                    // Empty MultiACE lane: do not retain stale filament data.
                    auto it = std::find_if(filaments.begin(), filaments.end(), [](const Preset &f) {
                        return f.is_compatible && f.is_system
                            && boost::algorithm::starts_with(f.name, "Generic PLA");
                    });
                    if (it == filaments.end()) {
                        it = std::find_if(filaments.begin(), filaments.end(), [](const Preset &f) {
                            return f.is_compatible && f.is_system
                                && boost::algorithm::starts_with(f.name, "Generic ");
                        });
                    }
                    std::string placeholder_name = (it != filaments.end()) ? it->name : filaments.first_visible().name;
                    result_colors.push_back(i < ams_filament_colors.size() && !ams_filament_colors[i].empty() ? ams_filament_colors[i] : "#CECECE");
                    result_color_types.push_back("1");
                    result_presets.push_back(placeholder_name);
                    result_multi_colors.push_back({i < ams_filament_colors.size() && !ams_filament_colors[i].empty() ? ams_filament_colors[i] : "#CECECE"});
                } else if (i < exist_presets.size()) {
                    // Beyond current tray count: keep existing filament
    ...
    update_multi_material_filament_presets(size_t(-1), !exact_count);
...
void PresetBundle::update_filament_count()
{
    ...
    const auto* inherits = printers.get_edited_preset().config.option<ConfigOptionString>("inherits");
    const bool dynamic_multiace = inherits != nullptr &&
        inherits->value.rfind("Snapmaker U1 MultiACE", 0) == 0;

    if (!dynamic_multiace && filament_presets.size() >= num_extruders)
        return;
...
void PresetBundle::update_multi_material_filament_presets(size_t to_delete_filament_id, bool expand_to_extruders)
{
    if (printers.get_edited_preset().printer_technology() != ptFFF)
        return;

    const auto& printer_cfg = printers.get_edited_preset().config;
    const bool is_multiace_u1 =
        printer_cfg.opt_string("printer_settings_id").find("Snapmaker U1 MultiACE") != std::string::npos ||
        printer_cfg.opt_string("inherits").rfind("Snapmaker U1 MultiACE", 0) == 0;
    if (is_multiace_u1)
        expand_to_extruders = false;
    ...
    if (expand_to_extruders && num_extruders > num_filaments) { // Verify validity of the current filament presets.
