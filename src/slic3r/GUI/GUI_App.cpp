...
public:
    SplashScreen(wxPoint pos = wxDefaultPosition)
        // No wxSPLASH_TIMEOUT â€” the splash is closed explicitly once MainFrame
        ...
        )
    {
        ...
        m_logo_bmp = *bmp_cache.load_png("splash_logo", 0, unsigned(sz.GetHeight() * 0.68));
    ...
    void OnPaint(wxPaintEvent& evt)
    {
        ...
        dc.Clear();
        if (m_logo_bmp.IsOk())
            dc.DrawBitmap(m_logo_bmp, (c_sz.GetWidth() - m_logo_bmp.GetWidth()) / 2, 0, true);
...
//     update_mode(); // !!! do that later
    ...
    if (is_gcode_viewer()) {
        ...
        // Restore the user preset's printer connection and MultiACE sync on startup.
...
void GUI_App::switch_printer_agent()
{
    ...
    if (preset_bundle->is_bbl_vendor()) {
        ...
        if (effective_agent_id == ORCA_PRINTER_AGENT_ID &&
            config.opt_string("printer_settings_id").find("Snapmaker U1 MultiACE") != std::string::npos) {
    ...
    if (current_agent_id != effective_agent_id) {
        ...
        // Agent is already correct; still select/create the machine for this preset host.
        select_machine(effective_agent_id);
    }
}
...
void GUI_App::select_machine(const std::string& agent_id)
{
    ...
    // Only user-created printer presets may bind to a print_host.
    // System presets are templates only and must never select/sync a physical printer.
    if (!preset.is_user()) {
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": ignoring system/default printer preset " << preset.name;
        return;
    }
...
void GUI_App::start_sync_user_preset(bool with_progress_dlg)
{
    ...
    m_sync_update_thread = Slic3r::create_thread(
        [this, progressFn, cancelFn, finishFn, t = std::weak_ptr<int>(m_user_sync_token)] {
            ...
            // must run on every exit path â€” otherwise an early bail-out would leak the modal
...
{
    // A manual sync's progress dialog is already on screen â€” ignore repeat triggers so a
    ...
    if (!m_user_sync_token) {
        ...
        // start the new sync once the old thread is joined â€” don't race it.
    ...
    // A helper thread joins the old thread off the UI thread â€” no freeze â€”
...
void GUI_App::force_push_conflicting_preset(const std::string& setting_id)
{
    ...
    // (We must NOT pull from the cloud here as the Pull path does â€” that would overwrite
    ...
    // name and stamp it onto the preset â€” otherwise sync_with_lock's `id == preset.setting_id`
...
void GUI_App::load_current_presets(bool active_preset_combox/*= false*/, bool check_printer_presets_ /*= true*/)
{
    ...
    if (printer_technology == ptFFF && !edited_printer_preset.config.opt_bool("single_extruder_multi_material")) {
        ...
        if (nozzle_diameter) {
            size_t filament_count = nozzle_diameter->values.size();
            const std::string multiace_inherits = edited_printer_preset.config.opt_string("inherits");
            const std::string multiace_settings_id = edited_printer_preset.config.opt_string("printer_settings_id");
            const bool preserve_multiace_filaments =
                edited_printer_preset.is_user() &&
                !edited_printer_preset.config.opt_string("print_host").empty() &&
                (multiace_inherits.find("MultiACE") != std::string::npos ||
                 multiace_settings_id.find("MultiACE") != std::string::npos);

            if (preserve_multiace_filaments && preset_bundle->filament_presets.size() > filament_count)
                filament_count = preset_bundle->filament_presets.size();

            preset_bundle->set_num_filaments(filament_count);
        }
    }
