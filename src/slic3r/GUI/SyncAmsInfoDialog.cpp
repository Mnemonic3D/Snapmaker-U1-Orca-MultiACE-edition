    ...
    , m_input_info(input_info)
{
    ...
    m_countdown_timer = new wxTimer(this);
...
FinishSyncAmsDialog::~FinishSyncAmsDialog()
{
    if (m_countdown_timer) {
        m_countdown_timer->Stop();
        delete m_countdown_timer;
        m_countdown_timer = nullptr;
    }
}

void FinishSyncAmsDialog::update_countdown_text()
{
    m_finish_text->SetLabel(
        m_success_text + wxString::Format("\nClosing in %d...", m_countdown_seconds));
    Layout();
    Fit();
}
