    ...
    bool DeviceManager::set_selected_machine(std::string dev_id)
    {
        ...
        if (it != my_machine_list.end())
        {
            if (selected_machine == dev_id)
            {
                ...
                // MultiACE: keep the existing exact-host LAN connection alive.
                // Re-selecting the same user preset must not disconnect the printer
                // while its filament inventory is being refreshed.
                ...
                if (active_printer_preset.is_user()) {
                    ...
                    if (!print_host.empty() &&
                        MachineObject::dev_id_from_address(print_host, port) == dev_id) {
                        BOOST_LOG_TRIVIAL(info)
                            << "set_selected_machine: same MultiACE host already selected, keep connection dev_id="
                            << dev_id;
