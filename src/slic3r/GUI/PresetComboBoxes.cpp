...
void PlaterPresetComboBox::update()
{
    ...
    if (m_type == Preset::TYPE_FILAMENT) {
        ...
        if (is_multiace_u1 && m_filament_idx >= 0) {
            const auto& ams_list = m_preset_bundle->filament_ams_list;
            auto tray_it = ams_list.begin();
            for (int i = 0; i < m_filament_idx && tray_it != ams_list.end(); ++i)
                ++tray_it;

            if (tray_it != ams_list.end()) {
