...
static string get_diameter_string(float diameter)
{
    ...
    if (s.find('.') != std::string::npos) {   // Remove trailing zeros, but keep at least one decimal if needed
        ...
        if (s.back() == '.') s += '0';        // Ensure "1." ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ "1.0"
    }
    return s;
}
    ...
    wxStaticBoxSizer(new wxStaticBox(parent, wxID_ANY, _L("Sliced Info")), wxVERTICAL)
{
    ...
    init_info_label(_L("Used Filament (mmÃƒâ€šÃ‚Â³)"));
...
struct Sidebar::priv
{
    ...
    // FIX (2026-08-21): these two MultiACE toggles used to be local variables in
    // Sidebar's constructor, with no member handle anything else could reach them
    // through afterward. Every other widget in this same titlebar row gets
    // re-scaled/re-laid-out in Sidebar::msw_rescale() (see below); these two
    // never did, so they went visually stale/truncated ("wacky") relative to
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(39 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    p->multiace_filament_view_toggle = new SwitchButton(
        p->m_panel_filament_title);
    p->multiace_filament_view_toggle->SetMaxSize({em_unit(this) * 13, -1});
    p->multiace_filament_view_toggle->SetLabels(_L("4 Head"), _L("All Colors"));
    p->multiace_filament_view_toggle->SetValue(!multiace_loaded_heads_view);
    p->multiace_filament_view_toggle->SetToolTip(
        _L("Show all available MultiACE colors or only filament currently loaded in the physical toolheads."));

    p->multiace_filament_view_toggle->Bind(
        wxEVT_TOGGLEBUTTON,
        [this](wxCommandEvent &e) {
            const bool loaded_heads = !p->multiace_filament_view_toggle->GetValue();

            wxGetApp().app_config->set(
                "multiace_filament_view_mode",
                loaded_heads ? "loaded" : "all");
            wxGetApp().app_config->save();

            e.Skip();
            CallAfter([this] { sync_ams_list(); });
        });
    ...
    // Rebuild filament titlebar layout:
    // left title | centered MultiACE controls | right-side filament controls.
    bSizer39->Detach(p->m_flushing_volume_btn);
    ...
    bSizer39->Add(multiace_center_sizer, 0, wxALIGN_CENTER_VERTICAL);
    bSizer39->AddStretchSpacer(1);

    bSizer39->Add(p->m_flushing_volume_btn, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, FromDIP(4));
    bSizer39->Add(del_btn, 0, wxALIGN_CENTER | wxLEFT, FromDIP(SidebarProps::IconSpacing()));
    bSizer39->Add(add_btn, 0, wxALIGN_CENTER | wxLEFT, FromDIP(SidebarProps::IconSpacing()));

    // Keep Sync Filaments immediately beside the filament/settings picker.
    bSizer39->Add(ams_btn, 0, wxALIGN_CENTER | wxLEFT, FromDIP(SidebarProps::WideSpacing()));
...
void Sidebar::msw_rescale()
{
    ...
    // FIX (2026-08-21): these two were never re-scaled/re-laid-out here, unlike
    // every other widget in this same titlebar row - they used to not even be
    ...
    // Force the titlebar row to re-flow at the widgets' new sizes now, rather
    // than depending on a later resize event that may never fire if the
    // panel's own outer size didn't change.
...
void Sidebar::on_filament_count_change(size_t num_filaments)
{
    ...
    while (i < num_filaments)
    {
        ...
        const auto& active_printer_preset = wxGetApp().preset_bundle->printers.get_edited_preset();
    if (!active_printer_preset.is_user())
        return;

    const auto& printer_cfg = active_printer_preset.config;
    if (printer_cfg.opt_string("print_host").empty())
        return;
        const bool is_multiace_u1 =
            (printer_cfg.opt_string("printer_settings_id").find("Snapmaker U1 MultiACE") != std::string::npos || printer_cfg.opt_string("inherits").rfind("Snapmaker U1 MultiACE", 0) == 0);
        if (!is_multiace_u1)
 ...
 *       ÃƒÂ¢Ã¢â‚¬ÂÃ…â€œÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ vt_slot (std::vector<DevAmsTray>) - virtual tray data for external filament
 *       ÃƒÂ¢Ã¢â‚¬ÂÃ¢â‚¬ÂÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ DevFilaSystem ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ DevAms ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ DevAmsTray - AMS unit hierarchy
 *       ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬Å“
...
std::map<int, DynamicPrintConfig> Sidebar::build_filament_ams_list(MachineObject* obj)
{
    ...
    const auto& multiace_printer_cfg = wxGetApp().preset_bundle->printers.get_edited_preset().config;
    const bool is_multiace_u1_for_trays =
        (multiace_printer_cfg.opt_string("printer_settings_id").find("Snapmaker U1 MultiACE") != std::string::npos || multiace_printer_cfg.opt_string("inherits").rfind("Snapmaker U1 MultiACE", 0) == 0);

    if (obj->ams_support_virtual_tray && !is_multiace_u1_for_trays) {
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (printer_cfg.opt_string("print_host").empty())
        return;
    const std::string printer_settings_id = printer_cfg.opt_string("printer_settings_id");
    if (printer_settings_id.find("Snapmaker U1 MultiACE") != std::string::npos || printer_cfg.opt_string("inherits").rfind("Snapmaker U1 MultiACE", 0) == 0) {
        const std::string print_host = printer_cfg.opt_string("print_host");
        const std::string port = printer_cfg.opt_string("printhost_port");
        if (!print_host.empty()) {
            const std::string target_dev_id = MachineObject::dev_id_from_address(print_host, port);
            MachineObject* target_obj = dev->get_local_machine(target_dev_id);
            if (!target_obj) {
                for (const auto& [id, machine] : dev->get_local_machinelist()) {
                    if (machine && MachineObject::dev_id_from_address(machine->get_dev_ip()) == target_dev_id) {
                        target_obj = machine;
                        break;
                    }
                }
    ...
    const bool is_multiace_u1 =
        (printer_settings_id.find("Snapmaker U1 MultiACE") != std::string::npos || printer_cfg.opt_string("inherits").rfind("Snapmaker U1 MultiACE", 0) == 0);

    if (!is_multiace_u1 && !wxGetApp().plater()->is_same_printer_for_connected_and_selected()) {
        return;
    }
    ...
    if (is_multiace_u1)
        sync_color_only = false;
    auto n              = wxGetApp().preset_bundle->sync_ams_list(unknowns, !sync_result.direct_sync, sync_result.sync_maps, enable_append, merge_info, sync_color_only, is_multiace_u1);
    wxString detail;
    ...
    // MultiACE: unloaded ACE slots keep a valid backing preset internally,
    // but display only their available slot color until the head is loaded.
    if (is_multiace_u1) {
        size_t combo_idx = 0;
        for (const auto& entry : list) {
            if (combo_idx >= p->combos_filament.size())
                break;

            const auto& tray = entry.second;
            const bool is_placeholder =
                tray.has("filament_slot_placeholder") &&
                tray.opt_bool("filament_slot_placeholder", 0u);

            if (is_placeholder) {
                p->combos_filament[combo_idx]->SetSelection(-1);
                p->combos_filament[combo_idx]->SetToolTip(wxEmptyString);
            }
            ++combo_idx;
        }
    }
    ...
    // MultiACE final placeholder display pass.
    // Run after preset refresh because select_preset/export may restore the backing preset label.
    if (is_multiace_u1) {
        // Preset refresh may rebuild the profile's original 16 controls.
        // Reapply the live MultiACE lane count before styling placeholders.
        wxGetApp().plater()->on_filament_count_change(n);
    ...
    // Persist the final synchronized filament selections across Orca restarts.
    if (!sync_color_only) {
        wxGetApp().preset_bundle->export_selections(*wxGetApp().app_config);
        wxGetApp().app_config->save();
    }
...
void Sidebar::update_printer_thumbnail()
{
    ...
    std::string model_name    = selected_preset.config.opt_string("printer_model");
    std::string thumbnail_key = model_name.empty() ? printer_type : model_name;
    if (printer_thumbnails.find(thumbnail_key) != printer_thumbnails.end()) // Use known cache first
        p->image_printer->SetBitmap(create_scaled_bitmap(printer_thumbnails[thumbnail_key], this, PRINTER_THUMBNAIL_SIZE.GetHeight()));
    else {
        ...
        */
                ...
                if (vendor_model.name == model_name) {
                    ...
                    if (boost::filesystem::exists(cover_path)) {
                        try {
                            ...
                            printer_thumbnails[thumbnail_key] = cover_path.string(); // Cache by selectable printer model
                            return;
                        } catch (...) {}
                    }
                }
        ...
        printer_thumbnails[thumbnail_key] = "printer_placeholder"; // Avoid unnecessary try
    }
}
...
void Plater::priv::replace_all_with_stl()
{
    ...
    for (unsigned int idx : volume_idxs) {
        ...
        if (new_path == input_path) {
            status += boost::str(boost::format(_L("ÃƒÂ¢Ã…â€œÃ¢â‚¬â€œ Skipped %1%: same file.\n").ToStdString()) % volume_name);
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " skipping replace volume : same filename " << new_path;
            continue;
        }

        if (!fs::exists(new_path)) {
            status += boost::str(boost::format(_L("ÃƒÂ¢Ã…â€œÃ¢â‚¬â€œ Skipped %1%: file does not exist.\n").ToStdString()) % volume_name);
        ...
        if (!replace_volume_with_stl(object_idx, volume_idx, new_path, "Replace with 3D file")) {
            status += boost::str(boost::format(_L("ÃƒÂ¢Ã…â€œÃ¢â‚¬â€œ Skipped %1%: failed to replace.\n").ToStdString()) % volume_name);
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " cannot replace volume : failed to replace with " << new_path;
            continue;
        }

        status += boost::str(boost::format(_L("ÃƒÂ¢Ã…â€œÃ¢â‚¬Â Replaced %1%.\n").ToStdString()) % volume_name);
    }
...
void Plater::_calib_pa_pattern(const Calib_Params& params)
{
    ...
    if (accels.empty()) {
        ...
        const auto msg{_L("INFO:") + "\n" +
                       _L("No accelerations provided for calibration. Use default acceleration value ") + std::to_string(long(accel)) + _L(u8"mm/sÃƒâ€šÃ‚Â²")};
...
void Plater::send_gcode_legacy(int plate_idx, Export3mfProgressFn proFn)
{
...

    {
        ...
        // current plate ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the bundle still carries every plate's gcode.
        if (use_3mf) {
...
void Plater::show_object_info()
{
    ...
    if (imperial_units)
        info_text += (boost::format(_utf8(L("Volume: %1% inÃƒâ€šÃ‚Â³\n"))) %volume_val).str();
    else
        info_text += (boost::format(_utf8(L("Volume: %1% mmÃƒâ€šÃ‚Â³\n"))) %volume_val).str();
