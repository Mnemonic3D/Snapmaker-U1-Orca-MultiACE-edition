...
#include "slic3r/Utils/MultiACE/MultiAcePostProcess.hpp"
	...
	if (m_print == m_fff_print) {
        if (m_upload_job.upload_data.use_3mf) {
            ...
            if (!m_fff_print->is_BBL_printer()) {
                ...
                const auto& mace_config = m_fff_print->full_print_config();
                auto* mace_enabled = mace_config.option<ConfigOptionBool>("multiace_native_postprocess");
                if (mace_enabled && mace_enabled->value) {
                    // Native path: rewrites source_path_str in place. Mutually
                    // exclusive with the external post_process script mechanism
                    // below, to avoid double-processing an already-rewritten
                    // gcode if a preset still has both configured during the
                    // transition off the external helper exe.
                    Slic3r::MultiACE::run_multiace_postprocess(source_path_str, mace_config);
                } else if (run_post_process_scripts(source_path_str, false, m_upload_job.printhost->get_name(), output_name_str,
                                             mace_config))
