...
// Orca: we use a different logic here. If SEMM is enabled, we set extruder count to 1.
    ...
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
        ...
        auto optgroup = page->new_optgroup(L("Basic information"), L"param_information", -1, true);
            ...
            if (extruder_idx == 0 && m_config->opt_string("printer_settings_id") == "Snapmaker U1 MultiACE") {
                Line note_line = Line{ "", "" };
                note_line.full_width = 1;
                note_line.widget = [this](wxWindow* parent) {
                    ogStaticText* note = nullptr;
                    return description_line_widget(parent, &note, _L("Example: ACE connected to a single extruder. Retraction when switching material must be set to 0."));
                };
                optgroup->append_line(note_line);
            }
